# CI-GCF 2025 Village Survey Maps

Progressive Web Apps for field surveys in 13 villages.

## Available Maps

- [Gobojango](gobojango_map.html)
- [Hukuntsi](hukuntsi_map.html)
- [Kisa](kisa_map.html)
- [Lehututu](lehututu_map.html)
- [Mabutsane](mabutsane_map.html)
- [Mathathane](mathathane_map.html)
- [Mmadinare](mmadinare_map.html)
- [Phuduhudu](phuduhudu_map.html)
- [Sehithwa](sehithwa_map.html)
- [Semolale](semolale_map.html)
- [Shorobe](shorobe_map.html)
- [Tsabong](tsabong_map.html)
- [Tshane](tshane_map.html)

## Features

- 📱 **Works Offline** - Add to home screen for true offline app
- 📍 **Real-time GPS** - High-accuracy location tracking
- 🗺️ **Interactive Maps** - Building footprints and survey zones
- 🔢 **Sampling Grids** - Target numbers for each area

## Usage

1. Click on your village map
2. Add to Home Screen (creates offline app)
3. Grant location permission
4. Tap location button (top-right) to start tracking

---

**Conservation International - Global Conservation Fund**  
Socio-Economic Baseline Survey 2025
